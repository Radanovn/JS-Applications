function toggle(e) {
 console.log(e.parentNode.getElementsByClassName('status')[0]);
  ...
  statusDiv.style.display =
    statusDiv.style.display === 'none' ? 'inline' : 'none';
}
...
(() => {
  ...
  async function renderCatTemplate() {
    const source = await fetch(
      'http://127.0.0.1:5500/02.%20HTTP%20Status%20Cats/templates/all-cats.hbs'
    ).then(res => res.text());
